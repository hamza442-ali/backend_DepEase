...
const createJenkinsJob = async (req, res) => {
  try {
    ...
    if (existingJob) {
      ...
      return res.status(400).json({ message: 'Job name already exists, please choose a different one' });
    } else{
...
data.deploymentLink = response.data.deploymentLink;
...
const deleteJenkinsJob = async (req, res) => {
  try {
    ...
    if (!job_name || !ProjectId) {
      return res.status(400).json({ error: 'job_name and ProjectId are required' });
    }
...
const deletedPipeline = await Pipeline.findOneAndDelete({ job_name, ProjectId });
...
module.exports = { loginToJenkins, createJenkinsJob,  deleteJenkinsJob,buildJenkinsJob,getDeploymentLink};
